...
config = {
    ...
    "l2reg":0.00000001, # weight decay
    "warmup_lr_factor":0.25,
    "max_epoch":200,
    "warmup_epochs":20,
    "blocks":[96]*3 + [192]*3 + [384]*9 + [768]*3,
    "num_workers":14,
}
...
def main():
  ...
  # Get dataloaders
  train_loader, val_loader, test_loader = getOxfordPetsDataloaders(config)

  # Build model
  model = ConvNext(3, 37, config["blocks"])
...
def train(model, train_loader, val_loader):
  ...
  wandb.login()
  wandb.init(project="Oxford Pets", name=run_name, config=config)
